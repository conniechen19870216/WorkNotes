...
#rt1=$(ftpVMbld $BUILDHOST $USERNAME $PASSWORD $APPDIR $SDMTARNAME)
#rt2=$(ftpVMbld $BUILDHOST $USERNAME $PASSWORD $APPDIR $OAMTARNAME)
#rt3=$(ftpVMbld $BUILDHOST $USERNAME $PASSWORD $SDMTOOLDIR $SDMTOOLTARNAME)
#if [[ $rt == "ok" ]] && [[ $rt1 == "ok" ]] && [[ $rt2 == "ok" ]] && [[ $rt3 == "ok" ]]; then
#	print " Download completed."
#else
#	print " There's some problem to download the file, please check,$VMTARNAME:$rt, $SDMTARNAME:$rt1, $OAMTARNAME:$rt2, $SDMTOOLTARNAME: $rt3."
#	exit 1
#fi

rt1=$(ftpVMbld $BUILDHOST $USERNAME $PASSWORD $APPDIR $COMPACTTARNAME)
if [[ $rt1 == "ok" ]] ; then
...
while [[ $initstat == "bad" ]]
do
	vm10run "/tmp/chk_init" > /tmp/chkinit.tmp
...
rt=$(scp2vm10 $targetdir/$OAMTARNAME /etc/SU/OAM400-service_logic.full.tar)
rt1=$(scp2vm10 $targetdir/$SDMTARNAME /etc/SU/SDM400-service_logic.full.tar)
rt2=$(scp2vm10 $targetdir/$SDMTOOLTARNAME /tmp/)
...
vm10run "mkdir /cust_use/SDM"
vm10run "cp /tmp/$SDMTOOLTARNAME /cust_use/SDM/"
vm10run "cd /cust_use/SDM; tar xvf $SDMTOOLTARNAME"
vm10run "/tmp/cfg_sdm_tool"
#vm10run "/tmp/gen_psw_apache"
vm10run "/tmp/addhost"
#vm10run "chmod 755 /usr/dhafw/shlib/*"
#vm10run "chmod -R 755 /usr/dhafw/data"
vm10run "chmod 755 /sn/cr/tracedir/"
vm10run "sed -e 's/ACCESS_CONTROL_ACTIVATED = 1/ACCESS_CONTROL_ACTIVATED = 0/' /usr/dhafw/data/pf_generic.cfg > /tmp/pf_generic.cfg"
vm10run "cp /tmp/pf_generic.cfg /usr/dhafw/data/pf_generic.cfg"
...
vm10run "cp /sn/sps/OAM400/bin/HSS/data/dataProfile/hssSubDataProfile.xml /root/hssSubDataProfile.xml.ofc"
scp2vm10 /home/bliu010/hssSubDataProfile.xml  /tmp/
vm10run "cp /tmp/hssSubDataProfile.xml /sn/sps/OAM400/bin/HSS/data/dataProfile/hssSubDataProfile.xml"
inst_spa OAM400
rt1=$?
if [[ $rt != "0" ]] || [[ $rt1 != "0" ]]; then
...
#sleep 180
sleep 30
subshl "op:status,spa=all;" |tee /tmp/opstat.tmp 
