...
#endif
{
   ...
   /* FID 17524.0 */
   inIsFirstChnlId(ntc, &cnStEvnt->chanId, pcb->cfg.chanIdIE); 
...
#endif
{
   ...
   mfTwoByte = FALSE;
   /* FID 17524.0 */
   if ((inIsFirstChnlId(ntc, &cnStEvnt->chanId, pcb->cfg.chanIdIE)) == TRUE)
   {
       if (cnStEvnt->chanId.eh.pres)
       {
           if ((cnStEvnt->chanId.chanMapType.pres) &&
   ...
   switch (ntc->evntType)
   {
      ...
      case MI_SETUPACK:           /* Setup Acknowledge, c, p, u */
         ...
         /* FID17555.0 Kavon */
         if (IN_CK_PLAYDIALTONE(ntc->waitForMoreInfo))
         {
            cnStEvnt->progInd.eh.pres = PRSNT_NODEF;
            cnStEvnt->progInd.location.pres = PRSNT_NODEF;
            cnStEvnt->progInd.location.val = LOC_NETINTER;  
            cnStEvnt->progInd.codeStand0.pres = PRSNT_NODEF;
            cnStEvnt->progInd.codeStand0.val = CSTD_CCITT;
            cnStEvnt->progInd.progDesc.pres = PRSNT_NODEF;
            cnStEvnt->progInd.progDesc.val = PD_IBAVAIL;
         }
...
#endif
{
   ...
   /* FID 17524.0 */
   inIsFirstChnlId(ntc, &ntc->sduSp->m.cnStEvnt.chanId, pcb->cfg.chanIdIE);
