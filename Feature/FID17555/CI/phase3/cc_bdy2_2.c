...
#endif
     /* BUG 93121
      * ACC service call, if receive a release indication during announcement from any
 ...
 )
{
        ...
        CC_IS_EXTBICC_BEARERSTATE(con,CC_INCOMING,CC_BICC_STATE_WT_COT))
    {
        ...
        if(con->ogPrecndCb)
        {
            PRECND_SET_CALL_FLAG2(con->ogPrecndCb, PRECND_FLAG_COT_RECVED);
        }

        /* FID 17373.0: precondition */
 ...
 )
{
  ...
  AnalyzeRoute_t     *pAnalyzeRoute         = NULLP;
  S16                 mgret                 = RFAILED;
  Bool                subAllNeeded          = FALSE;/* BUG: 94197 */
  ...
  /* bug94269 if transfer is in progress, need to clear it. Or, the new redirected call
   * will send INVITE to the refer to leg instead of the new redirected URI */
  if(con->transferParam)
  {
      SPutSBuf(ccInit.region, ccInit.pool, con->transferParam,
              (Size)(sizeof(ccTransferParam)));
      con->transferParam = NULLP;
  }

  ccRestoreEvntHold(con);
  /* BUG: 94197 Check to see if we need to SUB(*) */
  if (pAnalyzeRoute->inMgwInfo.pres  &&
      con->icBcm.mgCtx              &&
      (con->icBcm.mgSpId != pAnalyzeRoute->inMgwInfo.mgSpId[0]))
  {
    CCDP(AIN_DBG_LEVEL_1, "Received mg info not as before will SUB (*)\n");
    subAllNeeded  = TRUE;
  }
  ccExtractAnalyzeRteParam(pAnalyzeRoute, bcm);
  ccExtractSvcHeader(arSvcHeader, bcm, MSG_Analyze_Route);
  ...
  if ((CC_IS_CONTEXT_PRESENT(con)) && (con->ogRscVal))
  {
    if (subAllNeeded)
    {
      CCDP(AIN_DBG_LEVEL_1, "ccConE37S149 Sub (*) \n");
      ccQSubBasedOnCcn(con);
    }
    else
    {
 ...
 )
{
  ...
  if(pContinue->errorCode != FC_NOT_APPLICABLE)
  {
    ...
    if (CC_IS_CONTEXT_PRESENT(con))
    {
     ...
     if ((con->mgBlock->mgCtx) && (con->mgBlock->mgCtx->mgCmdQ != NULL)) /*Bug94302 +*/   
     {
      /* 28730 handle it differently */
      if (relRstRet == ROK)
      {
  ...
  else
  {
...

      {
        ...
        if (!CC_IS_CONTEXT_PRESENT(con))
        {
          ...
          /* og off-ans change */
          if (CC_OFFANS_OGRMTANS_PENDING(con)&&rmtSdp)
          {
            /*  If ACK/PRACK contains sdp answer and we have sent sdp
             *  offer before */
            CC_OFFANS_STATE_CHANGE(con, CC_OFFANS_STATE_NO_OP,
...
/********************************************************************90**
 $Log: cc_bdy2_2.c,v $
 Revision 1.580.2.10  2015/09/08 07:41:31  ashi
 BUG:94269

 Revision 1.580.2.9  2015/09/07 12:15:43  jfan
 BUG:94302

 Revision 1.580.2.8  2015/08/30 13:48:20  aliu002
 BUG:94197 Fix mgOptimization for two contact header 302 advance routing issue

 Revision 1.580.2.7  2015/08/27 02:10:47  jfan
 BUG:94143

 Revision 1.580.2.6  2015/02/04 08:28:54  aliu002
 BUG:93493 Remove the DP

 Revision 1.580.2.5  2014/12/23 02:57:06  yuqinzhang
 bug:93337 no additional MOD

 Revision 1.580.2.4  2014/12/09 09:00:24  brjiang
 BUG:93241 BRANCH-9-4-0 code change

 Revision 1.580.2.3  2014/11/26 08:40:20  ashi
 BUG:93262

 Revision 1.580.2.2  2014/11/25 13:41:41  kavonm
 BUG:93148

 Revision 1.580.2.1  2014/10/14 06:24:29  sahuang
 Bug:93121
